...
# added jit for faster computation, also silenced all warnings
import warnings
...
class ComplexityTest:
    ...
    def berlekamp_massey_algorithm(block_data):
        ...
        :return:
        """
        int_data = np.array([int(el) for el in block_data]).astype(float)
        return _berlekamp_massey_algorithm(block_data, int_data=int_data)
        # n = len(block_data)
        # c = zeros(n)
        # b = zeros(n)
        # c[0], b[0] = 1, 1
...
@njit
def _berlekamp_massey_algorithm(block_data, int_data):
    n = len(block_data)
    c = zeros(n)
    b = zeros(n)
    c[0], b[0] = 1, 1
